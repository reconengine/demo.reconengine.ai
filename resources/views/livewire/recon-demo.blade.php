<div class="container mx-auto px-4 py-4 md:py-12">
    <div class="flex flex-row items-center space-x-4 mb-8">
        ...
        <div class="space-y-2">
            <a class="text-lg underline hover:text-indigo-500 font-bold transition-colors" href="https://reconengine.ai/">Recon - World class recommendation engine brought to Laravel</a>
    ...
    <div class="mb-4">
        ...
        <input wire:model="search" type="search" name="email" id="email" class="shadow-sm focus:ring-indigo-500 focus:border-indigo-500 block w-full sm:text-sm border-gray-300 rounded-md" placeholder="Filter Articles">
    </div>
    ...
    @foreach($articles as $article)
        <div class="rounded-lg shadow-xl hover:shadow-md p-6 mb-8 bg-white transition-shadow duration-300">
            ...
            <div class="flex flex-col sm:flex-row sm:space-x-2 space-y-2 sm:space-y-0 items-center mt-4">
                <x-jet-button wire:click="displayRelatedArticles({{$article->id}})" wire:loading.attr="disabled" wire:target="displayRelatedArticles({{$article->id}})">
                    {{ __('View Related Articles') }}
                </x-jet-button>
                <x-jet-secondary-button wire:click="articleSelected({{$article->id}})" wire:loading.attr="disabled" wire:target="articleSelected({{$article->id}})">
                    {{ __('View More') }}
